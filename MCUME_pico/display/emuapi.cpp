...
static uint16_t readAnalogJoystick(void) {
    ...
    if (((nespad_state & DPAD_SELECT) != 0) && ((nespad_state & DPAD_LEFT) != 0)) joysval |= MASK_KEY_USER3;
    if (((nespad_state & DPAD_SELECT) != 0) && ((nespad_state & DPAD_RIGHT) != 0)) joysval |= MASK_KEY_USER4;
    if (((nespad_state & DPAD_SELECT) != 0) && ((nespad_state & DPAD_START) != 0)) joysval |= MASK_OSKB;
...
//    else topFile=curFile-(MAX_MENULINES/2);
            ...
            if (fileIndex >= topFile) {
                if ((i + topFile) < nbFiles) {
                    if ((i + topFile) == curFile) {
                        tft.drawTextNoDma(MENU_FILE_XOFFSET, i * TEXT_HEIGHT + MENU_FILE_YOFFSET, filename,
                                          RGBVAL16(0xff, 0xff, 0x00), RGBVAL16(0xff, 0x00, 0x00), true);
                        strcpy(selected_filename, filename);
                    } else {
                        tft.drawTextNoDma(MENU_FILE_XOFFSET, i * TEXT_HEIGHT + MENU_FILE_YOFFSET, filename,
                                          RGBVAL16(0xff, 0xff, 0xff), MENU_FILE_BGCOLOR, true);
                    }
                }
                i++;
            }
        ...
        tft.drawTextNoDma(48, MENU_JOYS_YOFFSET + 8, (emu_SwapJoysticks(1) ? (char *) "SWAP=1" : (char *) "SWAP=0"),
...
void emu_init(void) {
    ...
    if (keypressed & MASK_JOY2_DOWN) {
        ...
        tft.drawTextNoDma(64, 48, (char *) " AUTURUN file erased", RGBVAL16(0xff, 0xff, 0x00),
                          RGBVAL16(0xff, 0x00, 0x00), true);
        tft.drawTextNoDma(64, 48 + 24, (char *) "Please reset the board!", RGBVAL16(0xff, 0xff, 0x00),
                          RGBVAL16(0xff, 0x00, 0x00), true);
        emu_eraseConfig();
    } else {
        if (emu_readConfig()) {
